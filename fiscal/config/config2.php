...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "UPDATE fiscais SET nome = ?, email = ?, telefone = ?, cpf = ?, senha = ? WHERE fiscal_id = ?";
    $stmt = $conn->prepare($query);
    ...
    if ($stmt->execute()) {
        if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($_FILES["foto"]["tmp_name"], $target_file)) {
                $query = "UPDATE fiscais SET foto = ? WHERE fiscal_id = ?";
                $stmt = $conn->prepare($query);
...
<main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4" style="margin-left: 250px; padding-top: 50px;">
    ...
    <?php endif; ?>
            ...
            <div class="form-group">
                ...
                <input type="text" id="nome" name="nome" class="form-control" required value="<?php echo htmlspecialchars($fiscal['nome']); ?>">
            </div>
            <div class="form-group">
                <label for="email">Email:</label>
                <input type="email" id="email" name="email" class="form-control" required value="<?php echo htmlspecialchars($fiscal['email']); ?>">
            </div>
            <div class="form-group">
                <label for="telefone">Telefone:</label>
                <input type="tel" id="telefone" name="telefone" class="form-control" value="<?php echo htmlspecialchars($fiscal['telefone']); ?>">
            </div>
            <div class="form-group">
                <label for="cpf">CPF:</label>
                <input type="text" id="cpf" name="cpf" class="form-control" required value="<?php echo htmlspecialchars($fiscal['cpf']); ?>">
            </div>
            <div class="form-group">
