...
$stmt = $conn->prepare("SELECT * FROM funcionarios WHERE empresa_id = ? AND nome LIKE ?");
...
while ($funcionario = $result->fetch_assoc()) {
    $pdf->Cell(50, $linhaAltura, utf8_decode($funcionario['nome']), 1, 0, 'C');
    $pdf->Cell(50, $linhaAltura, utf8_decode($funcionario['email']), 1, 0, 'C');
    $pdf->Cell(40, $linhaAltura, utf8_decode($funcionario['telefone']), 1, 0, 'C');
    $pdf->Cell(40, $linhaAltura, utf8_decode($funcionario['cpf']), 1, 1, 'C');
}
