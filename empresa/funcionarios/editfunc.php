...
if (isset($_GET['id'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $update_sql = "UPDATE funcionarios SET nome = ?, email = ?, telefone = ?, cpf = ? WHERE func_id = ?";
        $update_stmt = $conn->prepare($update_sql);
        $update_stmt->bind_param("ssssi", $nome, $email, $telefone, $cpf, $func_id);
        
        if ($update_stmt->execute()) {
            header("Location: func.php?message=Funcionário atualizado com sucesso!");
            exit;
        } else {
            echo "Erro ao atualizar funcionário: " . $update_stmt->error;
        }
    }
} else {
    ...
    <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4" style="margin-left: 250px; padding-top: 50px;">
        ...
        <form method="POST">
            <div class="form-group">
                ...
                <input type="text" id="nome" name="nome" value="<?php echo htmlspecialchars($employee['nome']); ?>" required>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="text" id="telefone" name="telefone" value="<?php echo htmlspecialchars($employee['telefone']); ?>">
            </div>
            <div class="form-group">
                <label for="cpf">CPF:</label>
                <input type="text" id="cpf" name="cpf" value="<?php echo htmlspecialchars($employee['cpf']); ?>" required>
            </div>
